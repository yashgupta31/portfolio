...
const GmailForm = ({setShowGmailForm}) => {
  ...
  return (
    ...
    <Box bg={'rgba(0, 0, 0, 0.7)'}  position={'fixed'} top={'0rem'} w={'100%'} h={'100%'} zIndex={999} display={'flex'} justifyContent={'center'} alignItems={'end'}>
       <Box as='form' onSubmit={handleSubmit} bg={isDark?colors.dark.primary: colors.light.primary} color={isDark ? colors.dark.text : colors.light.text} w={isLargerThan400?'20rem': '17rem'} h={'27rem'} borderRadius={'md'} p={'1rem'} display={'flex'} flexDirection={'column'} justifyContent={'space-evenly'} position={'fixed'} bottom={'1rem'} transition={'0.6s'}>
